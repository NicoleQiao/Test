...
-- -----------------------------------------------------
-- Table `magnet_db`.`stretched_wire_system`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `magnet_db`.`stretched_wire_system` (
  `sw_run_id` INT NOT NULL AUTO_INCREMENT,
  `device_id` INT NULL,
  `sampling_rate` INT NULL,
  `speed` INT NULL,
  `accelerated_speed` INT NULL,
...
CREATE TABLE IF NOT EXISTS `magnet_db`.`magnet_design_parameter_table` (
  ...
  `ampere_turns` INT NULL COMMENT '励磁安匝数',
  `ampere_turns_each` INT NULL COMMENT '每磁极线圈匝数',
  `current` DOUBLE NULL COMMENT '励磁电流',
  `wire` VARCHAR(45) NULL COMMENT '导线规格',
  `current_density` DOUBLE NULL COMMENT '电流密度',
  `wire_length` INT NULL COMMENT '磁铁导线总长度',
  `resistance` INT NULL COMMENT '磁铁总电阻',
  `inductance` DOUBLE NULL COMMENT '磁铁电感',
  `voltage` DOUBLE NULL COMMENT '励磁电压',
  `consumption` DOUBLE NULL COMMENT '磁铁功耗',
  `c_pressure_drop` INT NULL COMMENT '冷却水压降',
  `c_channel_num` INT NULL COMMENT '并联水路数',
  `c_velocity` DOUBLE NULL COMMENT '冷却水流速',
  `c_flow` DOUBLE NULL COMMENT '冷却水流量',
  `c_temp` DOUBLE NULL COMMENT '水温升',
  `core_length` INT NULL COMMENT '铁芯长度',
  `core_section` INT NULL COMMENT '铁芯截面尺寸',
  `core_weight` INT NULL COMMENT '铁芯重',
  `copper_weight` INT NULL COMMENT '铜重',
...
CREATE TABLE IF NOT EXISTS `magnet_db`.`sws_data_table` (
  ...
  CONSTRAINT `FK_data_to_sws`
    FOREIGN KEY (`run_id`)
    REFERENCES `magnet_db`.`stretched_wire_system` (`sw_run_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `magnet_db`.`design_other_table`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `magnet_db`.`design_other_table` (
